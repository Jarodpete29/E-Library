...
if(isset($_POST['input'])){
  ...
  $query = "SELECT * FROM users WHERE (ID like '%{$input}%' OR Name like '%{$input}%' OR Course_Yrlevel like '%{$input}%' OR ID_Number like '%{$input}%' OR Email like '%{$input}%' OR Password like '%{$input}%' OR Account_type like '%{$input}%') AND VERIFIED =1 ORDER BY ID Desc";
